...
class CartProducts extends PolymerElement {
      ...
      <style include="shared-styles">
        :host {
         ...
         p{
          ...
          <h1>CART LIST</h1>
          <template is="dom-if" if="[[showTotalSum]]">
            <h2>
          ...
          <div><paper-button  on-click="addMore" class="btn" raised>Add More</paper-button></div> 

           <div>
           <template is="dom-if" if="[[showPurchaseButton]]">
         <h3><paper-button  on-click="purchase" raised>Purchase</paper-button><h3>
         </template>
          </div>  
  ...
  static get properties() {
    return {
      ...
      showTotalSum: {   //It will show totalsum based on cart length
        type: Number,
        value: 0
      }  
    };
  ...
  //set showTotalSum Based on cart length
  this.showTotalSum = this.cart.length;
